...
class Usu_model extends CI_Model
{
    ...
    function find_nick($u_nick,$id_usuario)
    {
        $query = sprintf("select * from usuarios where usu_nick='%s' and usu_id_usuario<>%s", $u_nick,$id_usuario);
        $row = $this->db->query($query);
        return $row;
        
    }
    
    function login($u_nick, $u_clave)
    {
